...
# Define putText function before it's used
def putText(frame, status, loc=(10, 40), color=(255, 0, 255)):
...
class Detect_Hands():
    def __init__(self, status=False, maxHands=2, detection_confidence=0.5, tracking_confidence=0.5):
        ...
        self.hands = self.mpHands.Hands(static_image_mode=self.status, 
    ...
    if status == 'Cursor':
        ...
        if fingers[1:] == [0, 0, 0, 0]:
            ...
            if len(landmarks_list) != 0:
                ...
                if fingers[0] == 0:
                    cv.circle(frame, (landmarks_list[4][1], landmarks_list[4][2]), 10, (0, 0, 255), cv.FILLED)
                    pyautogui.click()
    ...
    if status == 'Scroll':
        ...
        if len(landmarks_list) != 0:
            if fingers == [0, 1, 1, 1, 0]:
                putText(frame, status='Up', loc=(190, 40), color=(0, 0, 255))
                pyautogui.scroll(100)

            if fingers == [0, 1, 1, 1, 1]:
                putText(frame, status='Down', loc=(190, 40), color=(0, 0, 255))
                pyautogui.scroll(-100)
